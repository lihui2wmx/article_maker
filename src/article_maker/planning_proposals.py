...
from .claim_evidence import Claim, ClaimEvidenceLink
...
class PlanningProposalBuilder:
    ...
    def _claim_candidate(cls, claim: Claim) -> PlanningProposalCandidate:
        ...
        task = PlanningTask(
            ...
            scope=PlanningTaskScope(
                ...
                constraints=[
                    "Do not approve, reject, or reinterpret the Claim automatically."
                ],
    ...
    def propose_from_state(
        ...
        *,
        ...
        completed_experiment_ids = {
            run.experiment_id for run in runs if run.status is ExperimentRunStatus.COMPLETED
        }
        ...
        return [
            ...
            for candidate in sorted(
                candidates,
                key=lambda item: (item.reason.value, item.source_id, item.task.planning_task_id),
            )
            if candidate.task.planning_task_id not in existing_task_ids
        ]

    @staticmethod
    def _format_audit_failures(
        domain: str, findings: Iterable[object]
    ) -> list[str]:
        messages: list[str] = []
    ...
    def propose_from_repository(self) -> list[PlanningProposalCandidate]:
        ...
        audit_failures.extend(
            self._format_audit_failures("claim_evidence", self.claim_registry.audit())
        )
        audit_failures.extend(
            self._format_audit_failures("literature", self.literature_registry.audit())
        )
        audit_failures.extend(
            self._format_audit_failures("experiment", self.experiment_registry.audit())
        )
        audit_failures.extend(
            self._format_audit_failures("planning", self.planning_registry.audit())
        )
        if audit_failures:
            raise PlanningProposalSourceAuditError(
                "proposal source state failed repository audit: "
